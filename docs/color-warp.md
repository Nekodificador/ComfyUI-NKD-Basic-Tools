...
- **Drag a handle** to move the colors around it. `Pin` moves a single handle on
  its own; without it the whole spoke follows. Double-click resets a handle and
  `Reset all` starts over.
- **Drag the centre** for a global cast, and the whole web stretches with it.
- **Hold `Alt`** and the image shows you exactly which pixels a handle owns, so
  there's no guessing about what a move is going to hit.
...
- **Three radial scales.** `Neutrals` magnifies the near-grey band so you can
  ...
  everything.
- **Add or remove spokes and rings** for finer control, and turn on the `3D`
  scope, the `Luma` strip or `Trails` when you want to see what the grade is
  doing.
- **`save_lut`** also writes the grade to the output folder as a `.cube` you can
  load in Resolve, Premiere or anywhere else.
...
patch. The whole grade is a single lookup, so a 200-frame batch is graded
