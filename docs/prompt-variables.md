...
instance, and the prompt resolves once per item. Shift-click a chip, or use
`Randomize All`, to make that variable pick a random item instead, seeded so it
stays reproducible. The node shows the resolved prompt(s) on itself.
