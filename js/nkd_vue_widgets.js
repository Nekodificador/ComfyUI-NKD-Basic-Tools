...
function setup(node) {
  ...
  bakeBtn.onclick = async (e) => {
    ...
    const name = raw.trim();
    if (!name) return;
    const current = Number((strengthW == null ? void 0 : strengthW.value) ?? 1);
...
No: only the block shape is saved, and you set the strength on the loader.`
    ...
    const post = (overwrite) => api.fetchApi("/nkd/lora/save", {
      ...
      body: JSON.stringify({
        lora_name: src,
        rules: serialise(rows, threshold),
        name,
        overwrite,
        strength
      })
    ...
    try {
      ...
      note = `saved ${data.path}  (${data.tensors} of ${data.of} tensors` + (bake ? `, strength ${current} baked in)` : ")");
      commit();
    } catch (err) {
      window.alert(`Save failed: ${err}`);
    } finally {
